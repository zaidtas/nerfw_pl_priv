...
def main(hparams):
    ...
    for i in range(hparams.num_clients):
        ...
        hparams_list(i).root_dir = f'{hparams.root_dir}_{i}'
        hparams.num_epochs = hparams.num_epochs//hparams.num_clients
        
        system(i) = NeRFSystem(hparams,train_datasets[i],val_dataset)
        ...
        logger[i] = TestTubeLogger(save_dir="logs",
                            name=hparams.exp_name,
                            debug=False,
                            create_git_tag=False,
                            log_graph=False)

    hparams_central = hparams
    hparams_central.encode_t = False
    central_system = NeRFSystem(hparams_central)

    for i in hparams.num_epochs:
        for client_id in hparams.num_clients:
            #train the client model
            trainer = Trainer(max_epochs=1,
                      checkpoint_callback=checkpoint_callback[client_id],
                      resume_from_checkpoint=hparams.ckpt_path,
                      logger=logger[client_id],
            ...
            trainer.fit(system[client_id])

        #update the central model
        central_system.load_state_dict(system[client_id].state_dict())
