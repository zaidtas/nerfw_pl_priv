...
def setup_dataloader(hparams):
    ...
    partition_size = len(train_dataset) // hparams.num_clients
    ...
    return train_datasets, val_dataset, train_dataset

class NeRFSystem(LightningModule):
    def __init__(self, hparams, train_dataset=None,val_dataset=None,full_dataset=None):
        super().__init__()
    ...
    def setup(self, stage):
        if self.train_dataset is None:
            ...
            if self.hparams.dataset_name == 'phototourism':
                ...
                kwargs['use_mask'] = self.hparams.use_mask
...
def main(hparams):
    ...
    train_datasets,val_dataset, full_dataset = setup_dataloader(hparams)
    hparams_list = [hparams] * hparams.num_clients
    # import pdb; pdb.set_trace()
    for i in range(hparams.num_clients):
        #create copies of the model for each client
        # hparams_list[i].exp_name = f'{hparams.exp_name}_{i}'
        # hparams_list[i].ckpt_path = f'{hparams.ckpt_path}_{i}'
        hparams_list[i].num_epochs = hparams.num_epochs//hparams.num_clients
        
        # import pdb; pdb.set_trace()
        system = NeRFSystem(hparams,train_datasets[i],val_dataset,full_dataset)

        checkpoint_callback = \
        ModelCheckpoint(filepath=os.path.join(f'ckpts/{hparams.exp_name}/{i}',
                                               '{epoch:d}'),
                        monitor='val/psnr',
                        mode='max',
                        save_top_k=-1)

        logger = TestTubeLogger(save_dir="logs/{i}",
                            name=hparams.exp_name,
                            debug=False,
                            create_git_tag=False,
                            log_graph=False)
        trainer = Trainer(max_epochs=hparams_list[i].num_epochs,
                      checkpoint_callback=checkpoint_callback,
                      resume_from_checkpoint=hparams_list[i].ckpt_path,
                      logger=logger,
                      weights_summary=None,
                      progress_bar_refresh_rate=hparams_list[i].refresh_every,
                      gpus=hparams.num_gpus,
                      accelerator='ddp' if hparams_list[i].num_gpus>1 else None,
                      num_sanity_val_steps=1,
                      benchmark=True,
                      profiler="simple" if hparams_list[i].num_gpus==1 else None)
